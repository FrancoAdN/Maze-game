...
io.sockets.on("connection", (socket) => {
    ...
    for (let j = 0; j < users.length; j++) {
      if (users[j].ID == p_id) {
        if (en >= 0 && en < 2) {
          ...
          checkState();
        } else users[j].sock.emit("n_ent", null);
        break;
      }
    }
  });
  ...
  socket.on("pos", (data, p) => {
    ...
    for (let i = 0; i < games.length; i++) {
      if (games[i].id === p) {
        if (data.id == games[i].p_one.id) s = games[i].p_two.id;
        else s = games[i].p_one.id;
  ...
  socket.on("win", (pid, gid) => {
    ...
    for (let i = 0; i < games.length; i++) {
      if (games[i].id === gid) {
        if (pid == games[i].p_one.id) s = games[i].p_two.id;
        else s = games[i].p_one.id;
...
function sendGames() {
  for (let i = 0; i < users.length; i++) users[i].sock.emit("gList", games);
}
...
function checkState() {
  ...
  for (let i = 0; i < games.length; i++) {
    if (games[i].num_p === 2) {
      for (let j = 0; j < users.length; j++) {
        if (
          users[j].ID == games[i].p_one.id ||
          users[j].ID == games[i].p_two.id
        ) {
          if (users[j].ID == games[i].p_one.id)
            users[j].sock.emit("play", games[i].p_one);
          else users[j].sock.emit("play", games[i].p_two);
...
class Cell {
  ...
  checkNeighbors() {
    ...
    if (index(x - 1, y)) top = grid[x - 1][y];
    else top = index(x - 1, y);

    if (index(x, y + 1)) right = grid[x][y + 1];
    else right = index(x, y + 1);

    if (index(x + 1, y)) bot = grid[x + 1][y];
    else bot = index(x + 1, y);

    if (index(x, y - 1)) left = grid[x][y - 1];
    else left = index(x, y - 1);
...
function makeMaze(game, dif) {
  if (dif === 1) scl = 50;
  else if (dif === 2) scl = 25;
  else if (dif === 3) scl = 20;
